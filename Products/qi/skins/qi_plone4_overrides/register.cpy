...
if hasattr(context, 'getProject'):
    project = context.getProject()
    mailtool = getToolByName(context, 'MailHost')

    if hasattr(context, 'ProjectEmail'):
        emails = context.ProjectEmail
    else:    
        managers = project.getProjectUsers('managers')
        members = getToolByName(context, 'acl_users')
        emails = [members.getUserById(x).getProperty('email') for x in managers]

    message = """A new user has registered for QI Teamspace, because they want to join %s. The username is %s. 
You can go to %s to search and add them to your project as a project member. """ \
    % (str(project.title), username, "%s/members.html" % project.absolute_url())
    
    mailtool.send(message, emails, 'admin@qiteamspace.com', 'New User Registered')
